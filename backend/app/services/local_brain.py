...
def build_local_diet_plan(user_data: Dict[str, Any], targets: Dict[str, float], preferences: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    restrictions = normalize_keywords(prefs.get("dietary_restrictions"))
    preferred = normalize_keywords(prefs.get("preferred_foods"))
    model_hints = prefs.get("model_hints", {})
    diet_strategy = model_hints.get("diet_strategy")
    ...
    if diet_strategy == "high_protein_veg":
        vegetarian = True
        no_eggs = True
    if diet_strategy == "plant_performance":
        vegan = True
        vegetarian = True
        no_eggs = True
    ...
    if diet_strategy == "high_protein_non_veg":
        breakfast_protein = "Egg White Bhurji"
        lunch_protein = "Grilled Chicken"
        dinner_protein = "Chicken Rice Bowl"
    elif diet_strategy == "high_protein_veg":
        breakfast_protein = "Paneer Bhurji"
        lunch_protein = "Paneer Tikka"
        dinner_protein = "Dal + Paneer Bowl"
    elif diet_strategy == "plant_performance":
        breakfast_protein = "Tofu Scramble"
        lunch_protein = "Tofu Tikka"
        dinner_protein = "Soy Chunk Curry"
    ...
    dinner_carb = "Sauteed Vegetables" if user_data.get("goal") == "weight_loss" or diet_strategy == "fat_loss_high_fiber" else "Jeera Rice"
    snack_carbs = "Banana + Dates" if user_data.get("goal") == "endurance" or diet_strategy == "endurance_fuel" else "Banana Smoothie"
    ...
    return {
        ...
        "notes": f"Generated by the local FitAI Brain using {preference_text} while respecting {restriction_text} restrictions and staying close to your calorie and macro targets. Strategy: {diet_strategy or 'balanced_recomp'}.",
        "meals": meals,
    }
...
def rebalance_weekly_sets(weekly_schedule: Dict[str, Any], target_total_sets: int) -> int:
    exercises = []
    for session in weekly_schedule.values():
        for exercise in session.get("exercises", []):
            exercises.append(exercise)

    current_total_sets = sum(exercise.get("sets", 0) for exercise in exercises)
    delta = target_total_sets - current_total_sets
    guard = 0

    while delta != 0 and guard < 500 and exercises:
        changed = False
        ordered = exercises if delta > 0 else list(reversed(exercises))
        for exercise in ordered:
            if delta > 0 and exercise["sets"] < 5:
                exercise["sets"] += 1
                delta -= 1
                changed = True
            elif delta < 0 and exercise["sets"] > 2:
                exercise["sets"] -= 1
                delta += 1
                changed = True
    ...
    return sum(exercise.get("sets", 0) for exercise in exercises)
...
def build_local_workout_plan(user_data: Dict[str, Any], preferences: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    model_hints = prefs.get("model_hints", {})
    workout_strategy = model_hints.get("workout_strategy")
    duration_hint = model_hints.get("session_duration_min")
    set_hint = model_hints.get("total_weekly_sets")
    focus_hint = model_hints.get("focus_muscles")
    library = EXERCISES["home" if not equipment or has_any_keyword(equipment, ["home", "band", "bodyweight", "dumbbell"]) else "gym"]
    strategy_splits = {
        "full_body_foundation": ["Full Body", "Full Body", "Conditioning", "Full Body"],
        "upper_lower_progression": ["Upper Strength", "Lower Strength", "Push", "Pull", "Legs"],
        "push_pull_legs": ["Push", "Pull", "Legs", "Upper Strength", "Lower Strength", "Conditioning"],
        "conditioning_hybrid": ["Full Body", "Conditioning", "Lower Body", "Circuit", "Cardio"],
        "endurance_builder": ["Aerobic Base", "Support Strength", "Intervals", "Tempo", "Stability"],
    }
    split = strategy_splits.get(workout_strategy, SPLITS.get(goal, SPLITS["maintain"]))
    ...
    for day in WORKOUT_WEEK:
        ...
        if duration_hint:
            session["duration_minutes"] = int(duration_hint)
    ...
    if set_hint:
        total_sets = rebalance_weekly_sets(weekly_schedule, int(set_hint))

    return {
        "title": f"{workout_days}-Day {goal.replace('_', ' ').title()} Training System",
        "notes": f"Generated by the local FitAI Brain with progressive overload, recovery spacing, and beginner-friendly exercise selection. Strategy: {workout_strategy or goal}.",
        "focus_muscles": (focus_hint or unique_focus)[:6],
